...
class MACrossStrategyFast(VectorizedStrategyBase):
    """简单均线金叉死叉策略 - 快速版"""
    
    strategy_id = "ma_cross_fast"
    strategy_name = "MA金叉死叉策略-快速版"
...
try:
    ...
    strategy = MACrossStrategyFast(stock_code='000001')
    ...
    for event in engine.run_backtest(
        ...
        if event.get('type') == 'trade':
            ...
            print(f">>> 交易: {trade.get('date')} {trade.get('action')} {trade.get('stock_code')} @ {trade.get('price', 0):.2f}")
