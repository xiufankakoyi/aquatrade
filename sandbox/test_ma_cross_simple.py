...
class MACrossStrategy(VectorizedStrategyBase):
    ...
    def generate_signals_vectorized(
        ...
        print(f"\n[策略] 生成信号 - T={T}, N={N}")
        print(f"[策略] 目标股票: {self.target_stock}")
        print(f"[策略] 股票池前10个: {stock_codes[:10]}")
        
        # 找到目标股票的索引
        if self.target_stock not in stock_codes:
            print(f"[策略] 警告: 股票 {self.target_stock} 不在股票池中")
            return signals
            
        n_idx = stock_codes.index(self.target_stock)
        print(f"[策略] 目标股票索引: {n_idx}")
        
        # 使用 prepare_data 准备数据
        print(f"[策略] 调用 prepare_data...")
        self.prepare_data(preloaded_data, trading_dates, stock_codes)
        
        # 检查MA数据
        print(f"\n[策略] MA数据检查:")
        print(f"  self.ma5 shape: {self.ma5.shape if self.ma5 is not None else 'None'}")
        print(f"  self.ma10 shape: {self.ma10.shape if self.ma10 is not None else 'None'}")
        
        if self.ma5 is not None and self.ma10 is not None:
...
try:
    ...
    if strategy.signal_log:
        ...
        for sig in strategy.signal_log[:10]:  # 显示前10个
            print(f"    {sig['date']} {sig['type']} MA5={sig['ma5']:.2f} MA10={sig['ma10']:.2f}")
        if len(strategy.signal_log) > 10:
            print(f"    ... 还有 {len(strategy.signal_log)-10} 个信号")
    else:
        print("\n  没有发现任何信号")
