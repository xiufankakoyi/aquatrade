...
class DebugBacktestEngine(UnifiedBacktestEngine):
    def _execute_trades(self, current_time, stock_pool, signals, portfolio, cash, position_info, data_dict):
        ...
        if '000001' in signals:
            print(f"[TradeDebug] {date_str}: 000001信号 = {signals['000001']}")
            print(f"[TradeDebug] {date_str}: 当前持仓 = {portfolio}")
            print(f"[TradeDebug] {date_str}: 当前现金 = {cash}")
            print(f"[TradeDebug] {date_str}: 000001数据 = {data_dict.get('000001', {})}")
        ...
        if trades:
            for trade in trades:
                print(f"[TradeDebug] {date_str}: 执行交易 - {trade}")
        
        return result


print("=" * 70)
print("MA交叉策略回测 - 自己计算MA（调试版3）")
print("=" * 70)
