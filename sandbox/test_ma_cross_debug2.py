    ...
    def generate_signals_vectorized(
        ...
        for t in range(1, T):
            ...
            if ma5_stock[t-1] < ma10_stock[t-1] and ma5_stock[t] > ma10_stock[t]:
                ...
                print(f"  [信号] {current_date} t={t} 金叉")
            ...
            # 死叉
                ...
                print(f"  [信号] {current_date} t={t} 死叉")
        
        # 打印2025年的信号
        print(f"\n[策略] 2025年的信号:")
        for sig in self.signal_log:
            if sig['date'].startswith('2025'):
                print(f"  {sig['date']} {sig['type']} t_idx={sig['t_idx']}")
        
        return signals


print("=" * 70)
print("MA金叉死叉策略测试 - 调试版2")
print("=" * 70)
...
try:
    ...
    for event in engine.run_backtest(
        ...
        if event.get('type') == 'trade':
            ...
            print(f">>> 交易: {trade.get('date')} {trade.get('action')} {trade.get('stock_code')}")
