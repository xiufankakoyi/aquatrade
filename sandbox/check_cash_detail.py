...
for trade in trades:
    if trade['action'] == 'buy':
        cost = trade['price'] * trade['quantity'] * (1 + 0.0003)
        cash -= cost
        position += trade['quantity']
        entry_price = trade['price']
        print(f"{trade['date']}: 买入 {trade['quantity']}股 @ {trade['price']:.2f}")
        print(f"  成本: {cost:.2f}, 现金: {cash:.2f}")
    else:
        revenue = trade['price'] * trade['quantity'] * (1 - 0.001 - 0.0003)
        profit = (trade['price'] - entry_price) * trade['quantity'] - trade['price'] * trade['quantity'] * 0.001 - (trade['price'] + entry_price) * trade['quantity'] * 0.0003
        cash += revenue
        position -= trade['quantity']
        print(f"{trade['date']}: 卖出 {trade['quantity']}股 @ {trade['price']:.2f}")
        print(f"  收入: {revenue:.2f}, 盈亏: {profit:.2f}, 现金: {cash:.2f}")

print(f"\n【手动计算结果】")
print(f"  最终现金: {cash:.2f}")
print(f"  持仓: {position}")
print(f"  最终市值: {cash:.2f}")
print(f"  总收益: {(cash - initial_capital) / initial_capital * 100:.2f}%")

print(f"\n【回测引擎结果】")
print(f"  最终市值: {final_result['finalEquity']:.2f}")
print(f"  总收益: {final_result['totalReturn']:.2f}%")

print(f"\n【差异分析】")
diff = final_result['finalEquity'] - cash
print(f"  差异: {diff:.2f}")
