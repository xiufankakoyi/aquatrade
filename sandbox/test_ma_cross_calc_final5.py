...
try:
    ...
    # 手动计算收益率
    if len(trades_000001) >= 2:
        buy = [t for t in trades_000001 if t['action'] == 'buy'][0]
        sell = [t for t in trades_000001 if t['action'] == 'sell'][0]
        
        buy_cost = buy['price'] * buy['quantity'] * (1 + 0.0003)
        sell_revenue = sell['price'] * sell['quantity'] * (1 - 0.0003)
