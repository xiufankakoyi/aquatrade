...
def run_analysis(
    ...
    stock_info_dict = {}
    for row in stock_info.iter_rows(named=True):
        stock_info_dict[row['stock_code']] = {'industry': row.get('industry')}
    ...
    stock_codes = daily_df.select('stock_code').unique().to_series().to_list()
    ...
    for stock_code in stock_codes:
        try:
            stock_df = daily_df.filter(pl.col('stock_code') == stock_code).sort('trade_date')
            if len(stock_df) < 50:
                continue
