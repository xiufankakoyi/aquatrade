...
try:
    ...
    if hasattr(engine, '_date_to_idx'):
        ...
        print(f"\n引擎 _date_to_idx 样本:")
        sample_dates = ['2025-01-02', '2025-01-23', '2025-04-17', '2025-05-09']
    ...
    if hasattr(engine, '_signal_matrix'):
        ...
        print(f"\n信号矩阵中特定日期的信号:")
        for date_str, expected_t in test_dates:
            t_idx = date_to_idx.get(date_str, -1)
            if t_idx >= 0 and t_idx < signal_matrix.shape[0]:
                signal_val = signal_matrix[t_idx, 0]
                print(f"  {date_str} (t={t_idx}, expected={expected_t}): signal={signal_val}")
            else:
                print(f"  {date_str}: t_idx={t_idx} (out of range)")
